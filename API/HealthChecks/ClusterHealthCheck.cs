...
namespace Test.Host.Api.HealthChecks
{
       public class ClusterHealthCheck : IHealthCheck
    {
          private readonly IClusterClient _clusterClient;

        public ClusterHealthCheck(IClusterClient clusterClient)
        {
        ...
        {
            var manager = _clusterClient.GetGrain<IManagementGrain>(0);
            try
            {
                    
                var hosts = await manager.GetHosts(true);
